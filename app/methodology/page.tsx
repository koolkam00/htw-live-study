...
export default function MethodologyPage() {
  ...
  return <article className="prose">
    ...
    <p>The ten primary analyses are ranked by usefulness to runners and strength of the evidence. Nine use the personalized engine, with controls for a marathon, age, recorded gender, and relevant target or earlier times. Rank 2 uses the separate fast-start calculation described below. Each page shows only the controls that apply to its question. The research guide retains the twelve personalized comparisons, including its earlier three-group opening analysis.</p>
    <details className="methodology" id="fast-start-method">
      <summary>Rank 2: What happens after a very fast start?</summary>
      <div className="methodology-content">
        <p>Compare the first 10 km pace with the average marathon pace of the runner’s fastest eligible finish in the two strictly earlier calendar years. The current race and all same-year races are excluded from that benchmark. The analysis uses the database’s screened identity candidates, not name matching; those links are not independently verified people. An earlier best is not a measurement of current fitness or a declared goal.</p>
        <p>Opening change is 100 × (first 10 km pace ÷ earlier-best marathon pace − 1). Negative means faster. Six bands distinguish changes below −10%, −10% to below −5%, −5% to below −2%, −2% through 2%, above 2% through 5%, and above 5%. Exact decimal timing comparisons preserve these boundaries without rounding source times.</p>
        <p>Course, exact-age group, recorded gender and earlier-best time select the exact comparison group. Earlier-best bands are under 3:00, 3:00 to under 3:30, 3:30 to under 4:00, and 4:00 or longer, with an All option. There is no target-time or eventual-finish filter. Each opening group needs at least 100 eligible finishes with a recent benchmark; sparse selections are not broadened. Missing age or gender stays in All and is not inferred.</p>
        <p>Section curves show median pace changes relative to each runner’s earlier-best pace. Finish outcomes show the median and middle 80% of actual time differences from that earlier finish; the range is not a confidence interval or a prediction. Mean differences in the opening 10 km and the remaining distance add up to the mean whole-race difference. This accounting uses the earlier best’s average pace as a reference, not an alternative race the runner would have achieved.</p>
        ...
        <p>These are pooled observations from complete eligible finishers. The groups are not matched across race editions or adjusted for weather, terrain or changing fitness. A faster opening can accompany more late slowing and still a faster finish than an earlier best. Neither these comparisons nor the time accounting establish a causal time penalty, an optimal strategy or the chance of not finishing.</p>
        <Link href="/analyses/starting-pace">Open the fast-start analysis</Link>
      </div>
    </details>
    <h3>Shared personalized-engine methods</h3>
    <p>The following shared rules apply to the nine personalized primary views and the research guide. Rank 2 instead uses the six opening bands and exact filters above.</p>
    ...
    {PERSONAL_QUESTIONS.map(question => {
      ...
      return <details className="methodology" key={question.id}><summary>{question.title}</summary><div className="methodology-content"><p>{question.method}</p><Link href={primary ? analysisHref(primary) : `/research/personalized#guide-${question.id}`}>Open this personalized question</Link></div></details>;
    })}
    ...
    <ul>
      ...
      <li><strong>Fast-start section profiles:</strong> compare each section with the runner’s earlier-best marathon pace, then take the group median. This earlier reference differs from the current-race average used in full-course normalized profiles.</li>
    ...
    <p>Historical-performance charts can express change as 100 × (current finish ÷ earlier benchmark − 1). Opening change compares the first 10 km pace with that benchmark’s full-marathon average pace. Negative values mean faster. The archive and personalized guide’s three-group opening analyses classify starts as more than 2% faster, within 2%, or more than 2% slower. Rank 2 uses the six bands above and shows finish differences in minutes. These thresholds describe recorded pacing, not physiological boundaries.</p>
