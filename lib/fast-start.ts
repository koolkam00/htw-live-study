...
export type FastStartSelection = { city: string; age: string; gender: string; prior: string; band: string };
export type FastStartRow = Omit<FastStartSelection, 'band'> & { groups: FastStartGroup[] };
...
export const FAST_START_DEFAULT: FastStartSelection = { city: 'All courses', age: 'all', gender: 'all', prior: 'all', band: 'fast10' };
...
export function readFastStartSelection(search: string, start: Pick<FastStartStart, 'cities' | 'bands'>): FastStartSelection {
  ...
  return { city: start.cities.includes(city || '') ? city! : 'All courses',
    age: AGE_OPTIONS.includes(p.get('age') || '') ? p.get('age')! : 'all',
    gender: ['Men', 'Women'].includes(p.get('gender') || '') ? p.get('gender')! : 'all',
    prior: PRIOR_OPTIONS.some(row => row.id === prior) ? prior! : 'all',
...
export function fastStartSearch(selection: FastStartSelection): string {
  return '?' + new URLSearchParams({ race: selection.city, age: selection.age, gender: selection.gender, prior: selection.prior, opening: selection.band }).toString();
}
...
export function fastStartCharts(row: FastStartRow, focus: FastStartGroup, bands: FastStartStart['bands']): { pace: ChartSpec; finishes: ChartSpec; accounting: ChartSpec; onset: ChartSpec | null } {
  ...
  return {
    pace: { title: 'How the pace changed through the race', kind: 'line', unit: '% pace', xLabel: 'Section end (km)', xNumeric: true, sectionEnds: MARATHON_SECTION_ENDS,
      ...
      note: 'Median section pace relative to each runner’s earlier-best average pace. Below zero is faster; above zero is slower. These are different groups, not the same people trying two strategies. Lines connect section summaries; they do not locate changes within a section.' },
    finishes: { title: 'Finish time relative to the earlier best', unit: 'min', xLabel: 'Opening group',
      series: [{ key: 'p10', label: '10th percentile' }, { key: 'median', label: 'Median' }, { key: 'p90', label: '90th percentile' }],
      rows: row.groups.map(g => ({ label: name(g.band), p10: g.finish_delta_p10_s / 60, median: g.finish_delta_median_s / 60, p90: g.finish_delta_p90_s / 60, n_p10: g.n, n_median: g.n, n_p90: g.n })),
      note: 'All available opening groups for your filters. Negative minutes mean a faster finish. The range covers the middle 80% of observed changes; it is not a prediction or a confidence interval.' },
    accounting: { title: 'The early minutes and what followed', unit: 'min', xLabel: 'Part of the race', series: [{ key: 'value', label: 'Mean time difference' }],
      rows: [{ label: 'Opening 10 km', value: focus.opening_delta_mean_s / 60, n_value: focus.n }, { label: 'After 10 km', value: focus.remainder_delta_mean_s / 60, n_value: focus.n }, { label: 'Whole-race change', value: focus.finish_delta_mean_s / 60, n_value: focus.n }],
      note: 'Compare recorded times with covering each distance at the earlier best’s average pace. Negative means less time; positive means more. The first two averages add up to the whole-race average. This is time accounting, not minutes caused or lost by starting fast. The mean can differ from the median shown above.' },
...
export async function loadFastStartEvidence(start: FastStartStart, signal: AbortSignal): Promise<FastStartEvidence> {
  ...
  const response = await fetch((process.env.NEXT_PUBLIC_BASE_PATH || '') + '/data/fast-start/evidence.json?v=' + start.sha256, { signal });
