...
export default function PacingPreview({ answer }: { answer: GuideAnswer }) {
  ...
  const pace = (value: number) => Math.floor(Math.round(value * 60) / 60) + ':' + String(Math.round(value * 60) % 60).padStart(2, '0');
  return <div className="pacing-preview">
    ...
    <svg viewBox="0 0 480 248" role="img" aria-label="Typical pace by section for runners finishing between 3 hours 52 minutes 30 seconds and 4 hours 7 minutes 30 seconds. Explore analysis one for exact values.">
      ...
      <text x="42" y="237" fontSize="11" fill="currentColor" opacity=".6">First 5 km</text><text x={x(4)} y="237" textAnchor="middle" fontSize="11" fill="currentColor" opacity=".6">25 km</text><text x="442" y="237" textAnchor="end" fontSize="11" fill="currentColor" opacity=".6">Finish</text>
    </svg>
    <div className="preview-foot"><span>{answer.sample ? count(answer.sample.n) : 'Recorded'} finishes<br /><small>3:52:30–4:07:30 · minutes per km</small></span><Link href="/analyses/pacing-pattern" aria-label="Explore the pacing pattern analysis"><span aria-hidden="true">↗</span></Link></div>
  </div>;
}
