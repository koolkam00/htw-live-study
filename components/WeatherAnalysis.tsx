...
export default function WeatherAnalysis({ definition, candidate, evidence, questions }: { definition: WeatherDefinition; candidate: WeatherCandidate; evidence: WeatherEvidence; questions: WeatherDefinition[] }) {
  ...
  const lower = Math.min(-1, effect.low) - .3, upper = Math.max(1, effect.high) + .3;
  const position = (value: number) => (value - lower) / (upper - lower) * 100;
  const pp = (value: number) => `${value > 0 ? '+' : ''}${weatherNumber(value, 2)}`;
  const release = 'https://github.com/koolkam00/htw-live-study/releases/tag/' + evidence.input.release_tag;
  ...
  return <div className="analysis-layout">
    ...
    <article className="analysis-main weather-analysis">
      ...
      <section className="analysis-finding"><p className="eyebrow">What the data shows</p><h2>{weatherFinding(candidate)}</h2>
        ...
        <p className="comparison-context">{candidate.support.editions} race editions · {candidate.support.courses} courses · {cohortCount} complete finishes. Each edition has equal weight.</p><p className="study-meta">Source: <a href={release}>September 11, 2026 export</a>.</p><p className="coverage-notice">Compared within courses, accounting for start temperature, calendar year and the other modeled weather factors. Field composition and other differences can still matter.</p>
      </section>
      <section className="weather-estimate" aria-labelledby={`${controlId}-estimate`}><p className="eyebrow">The adjusted comparison</p><h2 id={`${controlId}-estimate`}>How much more did runners slow?</h2><div className="weather-effect-number"><strong>{pp(effect.estimate)}</strong><span>percentage points of late-race slowing</span></div>
      ...
      <details className="analysis-method"><summary>How we calculated this</summary><div>{evidence.methodology.map((paragraph, index) => <p key={index}>{unitText(paragraph, units)}</p>)}<p>Source export: <a href={release}>September 11, 2026</a>. The essential ten retain their September 7 input. <a href={archiveUrl}>Download all three screening results and edition values</a>. <a href="https://open-meteo.com/en/docs/historical-weather-api">Weather source definitions</a>.</p><p>All three weather questions were screened together. The download retains their estimates and uncertainty, including any that were too uncertain for a separate page.</p></div></details>
