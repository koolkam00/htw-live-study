...
export function FastStartResults({ row, focus, start, units }: { row: FastStartRow; focus: FastStartGroup; start: FastStartStart; units: UnitSystem }) {
  ...
  const charts = fastStartCharts(row, focus, start.bands);
  ...
  return <>
    <section className="analysis-finding fs-finding" aria-labelledby="fast-start-finding">
      ...
      <p>The median finish in this starting group was <strong>{Math.abs(focus.finish_delta_median_s) < 3 ? 'about the same as' : timeChange(focus.finish_delta_median_s).toLowerCase() + ' than'}</strong> the runner’s earlier recorded best. Late slowing and a slower overall result are different outcomes.</p>
      {steady && <p className="control-help">For openings within 2% of the earlier-best pace, {percent(100 * steady.slowdown_n / steady.n)} developed sustained slowdown ({count(steady.slowdown_n)} of {count(steady.n)} finishes with the same filters). The groups contain different runners and race editions.</p>}
      <p className="study-meta">{count(focus.n)} eligible finishes · {count(focus.editions)} race editions · {cityName(row.city)} · {row.age === 'all' ? 'All ages' : `Age ${row.age}`} · {row.gender === 'all' ? 'All recorded genders' : row.gender} · {PRIOR_OPTIONS.find(p => p.id === row.prior)!.label}</p>
    </section>
    <div className="fs-numbers">
      <div><strong>{timeChange(focus.finish_delta_median_s)}</strong><span>Median finish change</span><small>Compared with the earlier recorded best</small></div>
      ...
      <div><strong>{percent(100 - rate)}</strong><span>No detected sustained slowdown</span><small>Does not rule out milder or shorter slowing</small></div>
    </div>
    <p className="fs-caution">A fast opening is measured against an earlier result, not the runner’s current fitness. These groups mix fitness changes, race days, terrain and choices. The differences do not prove that the start caused the result.</p>

    <section className="fs-section" aria-labelledby="fs-pace-title"><p className="eyebrow">01 / Watch the race unfold</p><h2 id="fs-pace-title">Does the early speed last?</h2><p>Each section shows its typical pace relative to the earlier best’s average pace. Above zero means slower. {focus.band !== 'steady' && !row.groups.some(g => g.band === 'steady') && 'The similar-opening group is too small to draw a comparison line for these filters.'}</p><QuestionViz spec={charts.pace} unitSystem={units} /></section>

    <section className="fs-section" aria-labelledby="fs-onset-title"><p className="eyebrow">02 / Find the turning point</p><h2 id="fs-onset-title">Where did sustained slowing begin?</h2><p>Here, sustained slowdown means at least 25% slower than the runner’s {distanceLabel(5, units)}–{distanceLabel(20, units)} pace, lasting at least {distanceLabel(5, units)} after {distanceLabel(20, units)}.</p>
    ...
    <section className="fs-section" aria-labelledby="fs-finish-title"><p className="eyebrow">03 / Look at the finish</p><h2 id="fs-finish-title">How did the finish times compare?</h2><p>A runner can fade late and still beat an earlier best. Compare the median and range across every starting group with enough data for your filters.</p><QuestionViz spec={charts.finishes} unitSystem={units} /><p className="control-help">Only groups with at least {count(start.min_cell)} finishes are shown. Missing groups are not treated as zero.</p></section>

    <section className="fs-section" aria-labelledby="fs-minutes-title"><p className="eyebrow">04 / Follow the minutes</p><h2 id="fs-minutes-title">Did later sections give back the early minutes?</h2><p>For the <strong>{name.toLowerCase()}</strong> group, compare the first {distanceLabel(10, units)} and the rest of the race with an even pace based on the earlier best. That reference is not an expected or predicted finish.</p><QuestionViz spec={charts.accounting} unitSystem={units} /></section>
  </>;
}

export default function FastStartAnalysis({ start }: { start: FastStartStart }) {
  ...
  const [data, setData] = useState<FastStartEvidence | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [retry, setRetry] = useState(0);
  const initialRow = fastStartRow([start.initial], selection);
  useEffect(() => {
    const restore = () => { const next = readFastStartSelection(window.location.search, start); setSelection(next); setDraft(next); };
  ...
  useEffect(() => {
    ...
    loadFastStartEvidence(start, controller.signal).then(value => { if (!controller.signal.aborted) { setData(value); setLoading(false); } })
  ...
  return <div className="analysis-layout">
    ...
    <article className="analysis-main fs-page">
      ...
      <form className="comparison-controls" onSubmit={event => { event.preventDefault(); apply(draft); }}>
        <div className="controls-heading"><h2>Choose the runners and their start</h2><span>Observed races with earlier results</span></div>
        <div className="profile-fields fs-controls">
          ...
          <label>Earlier recorded best<select value={draft.prior} onChange={e => setDraft({ ...draft, prior: e.target.value })}>{PRIOR_OPTIONS.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}</select></label>
        ...
        <p className="control-help">Opening pace covers the first {distanceLabel(10, units)}. “Faster” means less time per {units === 'mi' ? 'mile' : 'kilometre'} than the average pace of that runner’s best recorded finish in the two earlier calendar years. These thresholds describe a start; they do not diagnose overreach.</p>
      </form>
      ...
      {pending ? <p className="loading-message">Loading the comparison…</p> : error ? <div className="feedback-error" role="alert"><p>{error}</p><button type="button" onClick={() => setRetry(n => n + 1)}>Try again</button></div> : row && focus ? <FastStartResults row={row} focus={focus} start={start} units={units} /> : <section className="empty-comparison"><h2>Not enough results for this starting group.</h2><p>This exact combination needs at least {count(start.min_cell)} finishes with eligible earlier results. Try another opening group or broaden the course, age, gender or earlier-time filters.</p><button className="button-secondary" type="button" onClick={() => apply(FAST_START_DEFAULT)}>Reset to all runners</button></section>}
      <section className="fs-method"><h2>What this can tell you</h2><p>The comparison follows {count(start.history_n)} eligible finishes with a usable recent recorded benchmark. It includes complete finishers only. Withdrawals, intended targets, fitness, training and fueling are not measured, and the same runner may contribute several races.</p><details><summary>Definitions, coverage and limits</summary><p>Candidate identities use the database’s screened links; they are not independently verified people. Earlier bests use eligible finishes in the two strictly earlier calendar years. No current-year result can supply the benchmark.</p><p>Every displayed group needs at least {count(start.min_cell)} finishes. Filters select the exact requested cohort; sparse selections are not silently widened. Age uses recorded exact ages from 18 to 89. Missing age or gender remains in All and is not inferred.</p><p>Sustained slowdown uses the study’s established 25% threshold over at least {distanceLabel(5, units)} after {distanceLabel(20, units)}, compared with {distanceLabel(5, units)}–{distanceLabel(20, units)} pace. Onset is a recorded section boundary. An isolated short final section cannot meet the distance requirement.</p><p>Neither finish-time differences nor the early/later time accounting establish a causal penalty. Weather, historical course changes, changing fitness and selection into complete linked results remain mixed together.</p></details><p><a href={(process.env.NEXT_PUBLIC_BASE_PATH || '') + '/data/fast-start/evidence.json'}>Download the analysis data</a> · <a href={sourceReleaseHref(start.release_tag)}>Open the source data</a> · <Link href="/methodology">Study methods</Link></p></section>
    ...
    <style jsx global>{`
      ...
      @media(max-width:420px) { .fs-controls { grid-template-columns:1fr; } }
    `}</style>
  </div>;
}
