...
export default function CheckpointExplorer({ summary, profile }: { summary: PersonalSummary; profile: Profile }) {
  ...
  return <section className="checkpoint-explorer">
    <form onSubmit={event => {
      ...
      <div className="profile-fields checkpoint-fields"><label>Distance reached<select value={checkpoint} onChange={event => { clearComparison(); setCheckpoint(Number(event.target.value)); setEdited(false); }}>{[20, 30, 35].map(km => <option key={km} value={km}>{km} km</option>)}</select></label><label>Elapsed time<input value={elapsedText} onChange={event => { clearComparison(); setElapsedText(event.target.value); setEdited(true); }} placeholder="2:50:00" spellCheck={false} inputMode="text" aria-describedby="elapsed-help" /></label><label>Last 5 km · optional<input value={sectionText} onChange={event => { clearComparison(); setSectionText(event.target.value); }} placeholder="e.g. 28:00" spellCheck={false} aria-describedby="elapsed-help" /></label></div>
      <p className="control-help" id="elapsed-help">Elapsed time: hours:minutes:seconds. Last 5 km: minutes:seconds.</p>
    ...
    {error && <div className="feedback-error" role="alert"><p>{error}</p>{request && <button type="button" onClick={() => setRequest(current => current ? { ...current, sequence: current.sequence + 1 } : current)}>Try again</button>}</div>}
