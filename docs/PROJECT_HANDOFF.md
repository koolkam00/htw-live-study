...
Evidence updated 2026-09-11 during takeover on `codex/documentation-handoff-20260911`, starting at commit `8cc2ada`. The base main commit was `140e5c66e7a433dfc3c6eab0563ec39845006d3f` (PR #28). This document distinguishes that inspected deployment, the export contract and later changes. Public-access work followed on `codex/public-data-access`. The ten-analysis redesign subsequently shipped in [PR #32](https://github.com/koolkam00/htw-live-study/pull/32), main commit `3e32bf8`, and was verified live. The units change described below is a later presentation update; that earlier deployment verification does not establish its publication status.
...
**Later September 11 update:** the compatible `private-export-20260911-0336` FULL export has now been downloaded and verified. A standalone [weather screen](WEATHER_ANALYSES.md) calculated all three proposed questions: warming and wind passed the prespecified takeaway rule, while humidity was withheld. Two additional pages use 3,120,690 finishes across 174 editions and 29 cities from that export. The original ten, existing extension packs and `analysis/release.json` remain September 7; `analysis/weather-release.json` pins the new weather input separately. The historical September 10 failure below has been superseded by a successful September 11 release run, not by a retry of the old run. See the weather document for exact evidence and local validation; publication is a separate status.
...
The display-unit update makes miles and minutes per mile the default, with a Miles / Kilometres switch shared across the ten analyses. `units=mi|km` in a shared URL takes precedence over the saved browser preference; a visitor with neither starts in miles. Miles mode displays elevation in feet. These are display conversions: calculations, source fields, numerical thresholds, cohorts and the September 7 release pin stay unchanged. Recorded timing sections remain intact: a 5 km section is shown as 3.11 miles, not interpolated into individual-mile splits. See [website unit semantics](WEBSITE_ARCHITECTURE.md#display-units).
...
- The ten-analysis navigation and expanded target range shipped in PR #32, main commit `3e32bf8`, and were verified on the public site. Its checked-in and deployed source release remained September 7. Display-unit work does not recalculate that pack or adopt another export.
...
The compatible September 11 export restores name/edition columns and packages audit sidecars separately. The next full refresh should rerun current analysis code against that export, audit coverage and linkage, then adopt the resulting packs and pin together. The earlier successful September 11 personalized artifact used the old 150–270-minute range and cannot replace the current 90–720-minute pack. The standalone weather work does not perform that broader refresh. Continue reconciling split/source/edition completeness and exact-age coverage with the [producer handoff](INGESTION_HANDOFF.md), treating its September 10 blockers as historical. The redesigned navigation shipped in PR #32 and miles display in PR #33; later changes require checks against their own named build.
