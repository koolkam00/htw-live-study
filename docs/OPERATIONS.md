...
Use [analysis/README.md](../analysis/README.md) for the full existing contract. This document records procedures, not actions executed by this documentation PR.
...
- September 10's failed run did not calculate outputs. The present asset list differs from what may have existed at failure; inspect rather than guessing. See KNOWN_ISSUES.md.
...
The source machine continues writing platform.sqlite. Consistent SQLite backup and integrity checks are documented in earlier release notes; do not copy a changing SQLite file blindly or replace live ingestion with a downloaded backup. Producer scripts, schedules, retries, recovery procedures and credentials are not available in this checkout. Obtain the [ingestion handoff](INGESTION_HANDOFF.md) before operating that environment.
